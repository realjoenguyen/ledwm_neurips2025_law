...
while [ $count -lt $max_iterations ]; do
    ...
    cmd="sh scripts/run_messenger_s1.sh $ids --jax.train_devices $seq_ids --jax.policy_devices 0 --run.server $arg1 --configs sent large_encoder large_decoder reward_grain sent_cache no_atten dense_image float32 long_read random_policy --envs.amount 50 --batch_size $batch_size"
    
    if [ $count -eq 0 ]; then
        echo "batch_size = $batch_size"
    else
        echo "FAIL. batch_size = $batch_size"
    fi
