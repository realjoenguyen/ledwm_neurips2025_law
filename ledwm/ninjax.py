...
def pure(fun, nested=False):
    ...
    The result is a pure function that is composable with JAX transformation.
    The pure function can be used as follows:
...
def grad(fun, keys, has_aux=False):
    ...
    state entries or modules.
    The transformed function returns a tuple containing
    the computed value, selected state entries, their gradients, and if
    applicable auxiliary outputs - immediate outputs - of the function."""
...
def _scope_method(method):
    """Ensure that the method is called within the name scope of the module.
    Call def scope to modify global var SCOPE
    """
...
class Module(object, metaclass=ModuleMeta):
    ...
    def get(self, name, *args, **kwargs):
        ...
        Save in self._submodules
        """

        assert "{" not in name, "Did you forget to format a string?"
        path = self.path + "/" + name
        if name in self._submodules:
            # cprint(f"[WARNING] EXISTING: get {name} from {self.path}!", "red")
