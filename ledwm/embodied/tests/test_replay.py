...
from ledwm.embodied.replay.reverb import Reverb
...
REPLAYS_UNLIMITED = [
    Uniform,
    Reverb,
    bind(PrioritizedSampler, zero_on_sample=False),
...
REPLAYS_LIMITED = [
    # bind(UniformWithOnline, batch=9999, online_fraction=0.1),
]
...
def _make_replay_config(replay_min_size=0, replay_type="uniform"):
    return Config(
        {
            "batch_length": 5,
            "batch_size": 10,
            ...
            "run": {
                "from_checkpoint": "",
                "overfit_eps": False,
                "script": "finetune_policy",
                "train_ratio": 20,
            },
            "replay": {
                "min_size": replay_min_size,
                "resume": False,
                "size": 100,
                "tolerance_rate": 5,
                "type": replay_type,
...
class TestReplay:
    def test_make_replay_defaults_min_size_to_batch_size(self):
        config = _make_replay_config()

        min_size = replays._resolve_replay_min_size(config, config.batch_size, False)

        assert min_size == config.batch_size

    def test_make_replay_uses_train_replay_min_size_when_configured(self):
        config = _make_replay_config(replay_min_size=3)

        min_size = replays._resolve_replay_min_size(config, config.batch_size, False)

        assert min_size == 3

    def test_prioritized_min_size_covers_unique_batch(self):
        config = _make_replay_config(replay_min_size=3, replay_type="prioritize")

        min_size = replays._resolve_replay_min_size(config, config.batch_size, False)

        assert min_size == config.batch_size

    def test_make_replay_does_not_apply_train_min_size_to_eval_replay(self):
        config = _make_replay_config(replay_min_size=3)

        min_size = replays._resolve_replay_min_size(config, 4, True)
    ...
    def test_restore_chunks_exact(self, tmpdir, Replay, length, capacity, chunks):
        ...
        assert len(replay) == num_items
        replay.save(wait=True)
        filenames = list(Path(tmpdir).glob("*.npz"))
        assert [filename.name for filename in filenames] == [bundlelib.FILENAME]
        saved_chunks = bundlelib.load(tmpdir)
        assert 0 < sum(chunk.length for chunk in saved_chunks) <= 30
        replay = Replay(length, capacity, directory=tmpdir, chunks=chunks)
        assert sorted(Path(tmpdir).glob("*.npz")) == sorted(filenames)
        assert len(replay) == num_items
        dataset = iter(replay.dataset())
        for _ in range(len(replay)):
            assert len(next(dataset)["step"]) == length
    ...
    def test_restore_chunks_workers(
        ...
        assert [filename.name for filename in filenames] == [bundlelib.FILENAME]
        saved_chunks = bundlelib.load(tmpdir)
        assert 0 < sum(chunk.length for chunk in saved_chunks) <= 50 * workers
        replay = Replay(length, capacity, directory=tmpdir, chunks=chunks)
        assert len(replay) == num_items
        dataset = iter(replay.dataset())
        for _ in range(len(replay)):
    ...
    @pytest.mark.parametrize(
        "length,capacity,overlap", [(1, 1, 0), (5, 10, 3), (10, 5, 2)]
    )
    ...
    @pytest.mark.parametrize("Replay", REPLAYS_QUEUES)
    @pytest.mark.parametrize(
        "length,capacity,overlap,workers", [(1, 10, 0, 2), (2, 10, 1, 2), (5, 30, 3, 4)]
    )
