...
class TestSampleTree:
    ...
    @pytest.mark.parametrize("branching", [2, 5, 16])
    def test_sample_batch_without_replacement(self, branching):
        tree = sampletree.SampleTree(branching, seed=0)
        for key in range(300):
            tree.insert(key, key + 1)
        keys = tree.sample_batch(200, replace=False)
        assert len(keys) == 200
        assert len(set(keys)) == 200

    def test_sample_batch_requires_enough_unique_entries(self):
        tree = sampletree.SampleTree(branching=4, seed=0)
        for key in range(10):
            tree.insert(key, 1.0)
        with pytest.raises(ValueError, match="Cannot sample 11 unique keys"):
            tree.sample_batch(11, replace=False)
