...
def acting(agent, env, replay, logger, actordir, args):
    ...
    logdir.mkdirs()
    print(f"actor.logdir | path={logdir}")
    ...
    print(f"actor.space | type=observation | value={env.obs_space}")
    print(f"actor.space | type=action | value={env.act_space}")
    ...
    def per_episode(ep):
        ...
        print(f"actor.episode | length={length} | score={score:.1f}")
        stats = {}
    ...
    if fill:
        print(f"actor.dataset_fill | steps={fill}")
    ...
    while step < args.steps:
        ...
        if should_sync(step):
            print("actor.checkpoint_sync")
            actor_cp.save()
            while not agent_cp.exists():
                print("actor.checkpoint_wait | reason=not_created")
                time.sleep(10)
            for attempts in range(10):
                try:
                    ...
                    print(f"actor.checkpoint_load_error | error={e}")
