...
class VLNEnv(embodied.Env):
    def __init__(
        ...
        config_opts = [
            ...
            dataset,
            "TASK_CONFIG.TASK.NDTW.SPLIT",
            dataset,
            "TASK_CONFIG.ENVIRONMENT.ITERATOR_OPTIONS.SHUFFLE",
            mode == "train",
            "TASK_CONFIG.ENVIRONMENT.ITERATOR_OPTIONS.GROUP_BY_SCENE",
            mode != "train",
            "TASK_CONFIG.ENVIRONMENT.ITERATOR_OPTIONS.CYCLE",
            mode == "train",
        ]
        if mode == "test":
        ...
        self.config = get_config(
            os.path.dirname(os.path.realpath(__file__)) + "/vln.yaml", opts=config_opts
        )
        self._env = make_env_fn(self.config, get_env_class(self.config.ENV_NAME))
    ...
    def obs_space(self):
        spaces = {
            k: embodied.Space(v.dtype, v.shape)
            ...
            new_space.update(
                {
                    "token": embodied.Space(
                        low=0, high=32100, shape=(), dtype=np.uint32
                    ),
        ...
        new_space.update(
            {
                ...
                "next_expert_ac": embodied.Space(
                    np.int32, (), -1, len(self._disc_act_space)
                ),
            }
        )
        return new_space
    ...
    def step(self, action):
        if self._done or action["reset"]:
            ...
            if self._num_eps < self._anneal_expert_eps:
                self._expert_ep = (
                    np.random.rand()
                    < self._use_expert
                    - (self._use_expert - self._min_use_expert)
                    / self._anneal_expert_eps
                    * self._num_eps
                )
        ...
        action = action[
            "action"
        ]  # possible actions: STOP, MOVE_FORWARD, TURN_LEFT, TURN_RIGHT
    ...
    def _embed(self, string):
        ...
        return (self.embed_cache[string], self.token_cache[string])
    ...
    def get_embed_text(self, ob):
        if len(self.tokens) > 0 and self.read_step >= len(self.tokens):
            ...
            if (
                self._use_descriptions
                and len(ob["descriptions"]) > 1
                and self._step > 0
            ):
    ...
    def preprocess_depth(self, depth):
        ...
        depth = (np.clip(depth, 0, 5.0) / 5.0 * 255).astype(
            np.uint8
        )  # Clip to 5m, convert to uint8
    ...
    def render_with_text(self, ob, instr_text, traj_id, ac):
        ...
        wrapped_text = "\n".join(
            [instr_text[i : i + max_len] for i in range(0, len(instr_text), max_len)]
        )

        draw.text(
            (0, 0), "Trajectory ID: {}, Mode: {}".format(traj_id, self._mode), (0, 0, 0)
        )
