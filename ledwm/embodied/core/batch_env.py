from typing import List
import numpy as np
from . import base
from typing import TYPE_CHECKING

if TYPE_CHECKING:
    from embodied.envs.from_gym import FromGym


class BatchEnv(base.Env):
    def __init__(self, envs, parallel):
        assert all(len(env) == 0 for env in envs)
        assert len(envs) > 0
        self._envs = envs
        self._parallel = parallel not in ("none", None, False)
        self._keys = list(self.obs_space.keys())

    @property
    def obs_space(self):
        return self._envs[0].obs_space

    @property
    def act_space(self):
        return self._envs[0].act_space

    @property
    def env_cache(self):
        return self._envs[0].env_cache

    def reset_game_config(self, **kwargs):
        assert kwargs is not None, kwargs
        return [env.reset_game_config(**kwargs) for env in self._envs]

    def create_game_config(self, **kwargs):
        return self._envs[0].create_game_config(**kwargs)

    # def reset_game_configs(self, configs):
    #     return [
    #         env.reset_game_config(**config) for env, config in zip(self._envs, configs)
    #     ]

    def __len__(self):
        return len(self._envs)

    def step(self, action):
        assert all(len(v) == len(self._envs) for v in action.values()), (
            len(self._envs),
            {k: len(v) for k, v in action.items()},
        )
        obs = []
        for i, env in enumerate(self._envs):
            act = {k: v[i] for k, v in action.items()}
            obs.append(env.step(act))
        if self._parallel:
            obs = [ob() for ob in obs]
        return {k: np.array([ob[k] for ob in obs]) for k in obs[0]}

    def render(self):
        return np.stack([env.render() for env in self._envs])

    def close(self):
        for env in self._envs:
            try:
                env.close()
            except Exception:
                pass
