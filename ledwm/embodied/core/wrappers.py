...
class ReadFirst(base.Wrapper):
    def __init__(self, env, duration, zero_obs=True):
        ...
        self.init_obs = None
        event_logger.debug(
            f"env.read_first | duration={self._duration} | "
            f"zero_observation={str(self._zero_obs).lower()}"
        )
    ...
    def step(self, action):
        ...
        if self._step < self._duration:
            ...
            assert self.init_obs is not None
            if self._step > 0:
                obs["is_first"] = False
            if self._zero_obs:
...
#     def __init__(self, env, duration, reset=True, timeout_reward=0.0, first_step=False):
#         super().__init__(env)
#         self._duration = duration
...
class OneHotAction(base.Wrapper):
    ...
    def step(self, action):
        if not action["reset"]:
            assert np.isclose(
                action[self._key].sum(), 1.0
            ), f"{action[self._key].sum()=}"
            assert np.isclose(
                action[self._key].min(), 0.0
            ), f"{action[self._key].min()=}"
            assert np.isclose(
                action[self._key].max(), 1.0
            ), f"{action[self._key].max()=}"
...
class CheckSpaces(base.Wrapper):
    ...
    def step(self, action):
        ...
        obs = self.env.step(action)
        for key, value in obs.items():
            if "language" in key or key.startswith("log_"):
                continue
    ...
    def _check(self, value, space, key):
        if not isinstance(
            value, (np.ndarray, np.generic, list, tuple, int, float, bool, str)
        ):
...
class ResizeImage(base.Wrapper):
    ...
    def _resize(self, image):
        ...
        image = image.resize(self._size, self._Image.NEAREST)  # type: ignore
...
class PadImage(base.Wrapper):
    def __init__(self, env, key, size=(16, 16)):
        """
        Pad the image as the top-left in the new image, pad zeroes for the rest
        """
        super().__init__(env)
        self._size = size
        self._keys = [key]
...
class StopAfterEpisodes(base.Wrapper):
    ...
    #             from google3.learning.deepmind.xmanager2.client import (
    #                 xmanager_api as xm,
    #             )
