...
class SpriteRegistry:
    ...
    def generate_id_number(self, key):
        count = len(self._live_sprites_by_key[key]) + len(
            self._dead_sprites_by_key[key]
        )
        return count + 1
    ...
    def with_stype(self, stype, include_dead=False):
        ...
        if stype in self.sprite_keys:
            ...
            return [
                s
                for _, sprites in self.groups(include_dead)
                for s in sprites
                if stype in s.stypes
            ]
    ...
    def set_state(self, state: dict):
        ...
        if len(deleted_ids) > 0:
            for key in self.sprite_keys:
                self._live_sprites_by_key[key] = [
                    sprite
                    for sprite in self._live_sprites_by_key[key]
                    if not sprite.id in deleted_ids
                ]
                self._dead_sprites_by_key[key] = [
                    sprite
                    for sprite in self._dead_sprites_by_key[key]
                    if not sprite.id in deleted_ids
                ]
            ...
            for sprite_state in sprite_states:
                ...
                if id in self._sprite_by_id:
                    ...
                    sprite = self.create_sprite(
                        key, id, pos=sprite_state["state"]["rect"].topleft
                    )
                    sprite.set_game_state(sprite_state["state"])
...
class Action:
    ...
    def __str__(self):
        _key_name = (
            lambda k: pygame.key.name(k)
            if pygame.key.name(k) != "unknown key"
            else str(k)
        )
...
class BasicGameLevel:
    ...
    # ! add this for AStarChaser
    def empty_sprites(self):
        # empty_blocks = []
        # for x in range(self.width):
        #     for y in range(self.height):
        #         pos = (x * self.block_size, y * self.block_size)
        #         if not any(self.sprites_at(pos)):
        #             empty_blocks.append(pos)
        # return empty_blocks
        # take all background sprites
        background_sprites = self.sprite_registry.with_stype("background")
        empty_sprites = []
        for sprite in background_sprites:
            if not any(self.sprites_at(sprite.rect.topleft)):
                empty_sprites.append(sprite)

        # TEST THIS
        assert len(empty_sprites) == len(background_sprites), (
            len(empty_sprites),
            len(background_sprites),
        )
    ...
    def get_possible_actions(self) -> Dict[Tuple[int], Action]:
        ...
        try:
            ...
            assert "avatar" in self.sprite_registry.classes, (
                self.sprite_registry.classes
            )
            print(
                [
                    parent.__name__
    ...
    def tick(self, action: Union[Action, int]):
        ...
        assert action in self.get_possible_actions().values(), (
            "Illegal action %s, expected one of %s"
            % (action, self.get_possible_actions())
        )
        if isinstance(action, int):
            action = Action(action)
...
class VGDLSprite:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.key: str = key  # e.g. background
        self.id: str = id  # e.g. background.1
    ...
    def get_game_state(self) -> SpriteState:
        state = {
            attr_name: copy.deepcopy(getattr(self, attr_name))
