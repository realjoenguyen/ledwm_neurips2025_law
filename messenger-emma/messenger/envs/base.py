...
class Grid:
    ...
    def __init__(self, layers, shuffle=True, record_manual_ids=False):
        """
        layers:
        ...
        if record_manual_ids:
            self.manual_ids = np.ones(
                (config.STATE_HEIGHT, config.STATE_WIDTH, layers)
            ) * (-1)
    ...
    def add(self, entity: Entity, position: Position, manual_id=None):
        """
        ...
        if hasattr(self, "manual_ids"):
            assert (
                manual_id is not None
            ), "manual_id must be provided if record_manual_ids is True."
            self.manual_ids[position.y, position.x, self.order[self.entity_count]] = (
                manual_id
            )
        self.entity_count += 1
