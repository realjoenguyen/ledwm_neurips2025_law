...
export PYTHONPATH="$repo_root:$repo_root/messenger-emma${PYTHONPATH:+:$PYTHONPATH}"

usage() {
  cat <<'USAGE'
Usage:
  scripts/run_s1.sh --gpus IDS --batch-size N|auto [options] [-- train flags...]
...
Options:
  ...
  --configs NAMES     Config stack. If omitted, uses the default S1 train stack.
...
Examples:
  ...
  scripts/run_s1.sh --gpus 1 --batch-size auto -- --batch_length 50
  scripts/run_s1.sh --gpus 0,1 --batch-size 80 --server opt -- --run.from_checkpoint ckpt
USAGE
}
...
resolve_auto_batch_size() {
  ...
  local first_fail_units max_fit selected probe_root probe_log
  local gpu_signature code_revision dirty_hash request_hash cache_root cache_key cache_file
  local used
  ...
  gpu_signature=$(nvidia-smi -i "$gpu_ids" \
    --query-gpu=name,memory.total --format=csv,noheader,nounits | tr '\n' ';')
  code_revision=$(git -C "$repo_root" rev-parse HEAD 2>/dev/null || true)
  dirty_hash=$(git -C "$repo_root" diff --no-ext-diff --binary -- ledwm scripts/run_s1.sh |
  ...
  request_hash=$(printf '%s\n' \
    "$gpu_signature" "$num_gpus" "$code_revision" "$dirty_hash" \
    "${configs[*]}" "${shape_args[*]}" \
  ...
  cache_root=${LEDWM_AUTOBATCH_CACHE_DIR:-${JAX_COMPILATION_CACHE_DIR%/*}/ledwm_s1_autobatch}
  ...
  if [[ -s $cache_file ]] && read -r max_fit <"$cache_file" &&
    ...
    probe_batch() {
      local value=$1
      local probe=(bash "$script_dir/run_s1.sh" --gpus "$gpu_ids" --batch-size "$value")
    ...
    last_fit_units=0
    first_fail_units=$((high_units + 1))
    while ((candidate_units <= high_units)); do
      candidate=$((candidate_units * quantum))
      if probe_batch "$candidate"; then
        last_fit_units=$candidate_units
        echo "autobatch.fit | batch_size=$candidate" >&2
        ((candidate_units == high_units)) && break
        candidate_units=$((candidate_units * 2))
        ((candidate_units > high_units)) && candidate_units=$high_units
      else
        first_fail_units=$candidate_units
    ...
    candidate_units=$((last_fit_units + 1))
    high_units=$((first_fail_units - 1))
    while ((candidate_units <= high_units)); do
...
python_hash_seed=${PYTHONHASHSEED:-0}
fast_train_metrics=${LEDWM_FAST_TRAIN_METRICS:-1}
fast_optimizer_metrics=${LEDWM_FAST_OPTIMIZER_METRICS:-1}
skip_adam_metrics=${LEDWM_SKIP_ADAM_METRICS:-1}
skip_train_outs=${LEDWM_SKIP_TRAIN_OUTS:-1}
jax_env+=("TF_CPP_MIN_LOG_LEVEL=$tf_cpp_min_log_level")
jax_env+=("TF_ENABLE_ONEDNN_OPTS=$tf_enable_onednn_opts")
jax_env+=("PYTHONHASHSEED=$python_hash_seed")
jax_env+=("LEDWM_FAST_TRAIN_METRICS=$fast_train_metrics")
jax_env+=("LEDWM_FAST_OPTIMIZER_METRICS=$fast_optimizer_metrics")
jax_env+=("LEDWM_SKIP_ADAM_METRICS=$skip_adam_metrics")
jax_env+=("LEDWM_SKIP_TRAIN_OUTS=$skip_train_outs")
...
if [[ $dry_run == 1 || $dry_run == true ]]; then
  echo "CONDA_ENV=${LEDWM_ACTIVE_CONDA_ENV:-unknown}"
...
export LEDWM_FAST_TRAIN_METRICS=$fast_train_metrics
export LEDWM_FAST_OPTIMIZER_METRICS=$fast_optimizer_metrics
export LEDWM_SKIP_ADAM_METRICS=$skip_adam_metrics
export LEDWM_SKIP_TRAIN_OUTS=$skip_train_outs
