...
usage() {
  cat <<'USAGE'
Usage:
  scripts/run_s3.sh --gpus IDS --batch-size N|auto [options] [-- train flags...]
...
Options:
  ...
  --configs NAMES     Config stack. If omitted, uses the default S3 train stack.
...
Examples:
  scripts/run_s3.sh --gpus 0 --batch-size 40
  scripts/run_s3.sh --gpus 1 --batch-size auto -- --batch_length 60
  scripts/run_s3.sh --gpus 0,1 --batch-size 80 --server dgx -- --run.from_checkpoint ckpt
USAGE
}
...
resolve_auto_batch_size() {
  ...
  local first_fail_units max_fit selected probe_root probe_log probe_policy=1
  ...
  dirty_hash=$(git -C "$repo_root" diff --no-ext-diff --binary -- ledwm scripts/run_s3.sh |
  ...
  for arg in "${train_args[@]}"; do
    ...
    case "$arg" in
      ...
      --envs.amount|--replay.size|--replay.min_size|--run.train_ratio)
        skip_shape_value=1
        ;;
      --run.compile_only=*|--jax.profiler=*|--logdir=*|--use_wandb=*|\
      --envs.amount=*|--replay.size=*|--replay.min_size=*|--run.train_ratio=*)
  ...
  request_hash=$(printf '%s\n' \
    s3 "$gpu_signature" "$num_gpus" "$code_revision" "$dirty_hash" \
    "${configs[*]}" "${shape_args[*]}" \
  ...
  cache_root=${LEDWM_AUTOBATCH_CACHE_DIR:-${JAX_COMPILATION_CACHE_DIR%/*}/ledwm_s3_autobatch}
  ...
  if [[ -s $cache_file ]] && read -r max_fit <"$cache_file" &&
    ...
    probe_batch() {
      local value=$1
      local status=0
      local probe=(bash "$script_dir/run_s3.sh" --gpus "$gpu_ids" --batch-size "$value")
      ...
      echo "autobatch.probe | batch_size=$value | gpus=$gpu_ids | policy=$probe_policy" >&2
      LEDWM_CAPACITY_PROBE_POLICY=$probe_policy \
        DRY_RUN=0 WANDB_MODE=disabled "${probe[@]}" >"$probe_log" 2>&1 || status=$?
      probe_policy=0
      return "$status"
    }
