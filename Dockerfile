...
RUN conda create -n ledwm python=3.8
# Automatically use the conda env for any RUN commands
SHELL ["conda", "run", "-n", "ledwm", "/bin/bash", "-c"]
...
# COPY ledwm/env_vln.yml /environment.yml
...
COPY ledwm /ledwm
WORKDIR ledwm
RUN chown -R 1000:root /ledwm && chmod -R 775 /ledwm
RUN pip install -e /ledwm
